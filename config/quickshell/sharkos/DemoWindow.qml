...
// Metaball / glass-material test bench (milestones M1+M2). Two ways in:
//   SHARKOS_DEMO=1 quickshell -c sharkos      standalone (no bar)
//   qs -c sharkos ipc call demo toggle        toggle inside the running shell
// Drag the two blobs toward the top bar or each other and watch the necks
// form and break. Same GlassCanvas + shaders as the real shell.
FloatingWindow {
    id: win
    ...
    Item {
        ...
        Image {
            anchors.fill: parent
            source: win.theme.wallpaper !== "" ? "file://" + win.theme.wallpaper : ""
    ...
    GlassCanvas {
        ...
        coverage: coverage
        background: bgView
        tint: win.theme.glassTint !== "" ? win.theme.glassTint : "#66101820"
